...
BUMP ?= minor
RELEASE_FROM_BRANCH ?= origin/dev
...
checkdependencies: $(REPORTS_DIR) virtual-environment ## check dependencies meet licence rules
	poetry run dependency-check --project ${PROJECT_NAME} --scan . -f JSON -f HTML -f XML --prettyPrint -o $(REPORTS_DIR)/ --exclude '.git/**' --exclude '.venv/**' --exclude '**/__pycache__/**' --exclude "$(REPORTS_DIR)"'/**' --disableAssembly --nodeAuditSkipDevDependencies
...
dependency-report: $(REPORTS_DIR) virtual-environment ## check dependencies meet licence rules
	# Use single quotes: https://github.com/jeremylong/DependencyCheck/issues/2099#issuecomment-579955128
	poetry run dependency-check --project ${PROJECT_NAME} --scan . -f JSON -f HTML -f XML --prettyPrint -o $(REPORTS_DIR)/ --exclude '.git/**' --exclude '.venv/**' --exclude '**/__pycache__/**' --exclude "$(REPORTS_DIR)"'/**' --disableAssembly --nodeAuditSkipDevDependencies
...
$(VENV)/bin/sonar-scanner: virtual-environment
	poetry run bin/install_sonar_scanner.py --venv=$(VENV)

release-old: ## package and upload a release
...
release: ## make a release
	bin/release $(BUMP) $(RELEASE_FROM_BRANCH)
...
docs:
	env PROCESS=generate_helm poetry run python deployment/create_infrastructure.py
	(cd deployment/helm ; helm template --debug --namespace doc --generate-name ai-backend > full.yaml)
	poetry run bin/generate-docs helm deployment/helm/full.yaml
